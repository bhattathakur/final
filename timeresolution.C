{
  ...
  const char * pdfetrue="resolutionvstime.pdf"; //pdf file location
  char * inputdata="compiledtimeresolution.dat";
  c=new TCanvas();
  c->SetGrid();
  c->SetFillColor(20);
  graph_expected=new TGraphErrors(inputdata,"%lg%lg%lg%lg","");//time, resolution, runtime/2,error in resolution
  graph_expected->SetTitle("Resolution vs Time  Plot;time (s); resolution (eV);");
